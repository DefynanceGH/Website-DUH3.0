...
`;
const Heading = tw(SectionHeading)`font-bold w-full ml-6 text-left text-primary-100`;
...
const Card = styled.div`
  ${tw`flex flex-col mx-auto max-w-xs px-6 border-2 bg-white rounded-lg mt-12`}
  .imageContainer {
...
const PrimaryAction = styled(PrimaryButtonBase).attrs({as: "a"})`
  ${tw` px-16 text-white no-underline rounded bg-primary-600 mx-auto inline-block mb-10 text-sm font-semibold hocus:border-solid hocus:bg-white hocus:text-primary-600 focus:shadow-outline
  `}
`
  ...
  return (
    <Container>
      <ThreeColumnContainer>
        <Heading tw="my-0 py-0"><p tw="font-semibold text-black text-base my-0 py-0">LIVE LIFE ON YOUR TERMS</p>Features and Benefits</Heading>
        {cards.map((card, i) => (
          <Column key={i}>
            <Card>
      ...
      <div tw="flex">
        <PrimaryAction tw="text-xl" target="_blank" href="http://portal.defynance.com/" >Get an ISA Quote</PrimaryAction>
      </div>
    </Container>
  );
};